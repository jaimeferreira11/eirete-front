import {
  ChangeEvent,
  FC,
  MutableRefObject,
  useCallback,
  useMemo,
  useState,
} from 'react';

import { useTranslation } from 'next-i18next';

import { SearchOutlined } from '@mui/icons-material';
import { Box, InputAdornment, TextField, Typography } from '@mui/material';
import debounce from 'debounce';

import {
  FullScreenLoading,
  IListGenericaPagination,
  ListGeneric,
} from '@components/ui';
import { ICategoriaMovimiento, ListPaginationOptions } from '@core/interfaces';
import { useCategoriaMovimientosPaginado } from '@lib/hooks/swr/useCategoriaMovimientos';
import { useEffect } from 'react';
import { parseCategoriaMovimientosToItemList } from 'src/utils';
import { KeyedMutator } from 'swr';
import { CategoriaMovimientoDetailPlaceHolder } from './CategoriaMovimientoDetailPlaceHolder';
import { CategoriaMovimientoEditView } from './CategoriaMovimientoEditView';

interface Props {
  tipo: 'activos' | 'inactivos';
  mutateRef: MutableRefObject<any>;
  children?: React.ReactNode;
}

export const CategoriaMovimientoTab: FC<Props> = ({ tipo, mutateRef }) => {
  const { t } = useTranslation('categoriaMovimientosABM');

  const [search, setSearch] = useState('');

  const [pagination, setPagination] = useState<ListPaginationOptions>({
    desde: 0,
    limite: 10,
    currentPage: 0,
  });

  const onPageChange = useCallback((page: number): void => {
    setPagination((prev) => ({
      ...prev,
      desde: page * prev.limite,
      currentPage: page,
    }));
  }, []);

  const onRowsPerPageChange = useCallback((pageSize: number): void => {
    setPagination((prev) => ({
      ...prev,
      desde: 0,
      limite: pageSize,
      currentPage: 0,
    }));
  }, []);

  const [categoriaSelected, setEditCategoria] = useState<
    ICategoriaMovimiento | undefined
  >(undefined);

  const {
    categorias,
    isLoading,
    total,
    mutate,
  }: {
    categorias: ICategoriaMovimiento[];
    isLoading: boolean;
    total: number;
    mutate: KeyedMutator<any>;
  } = useCategoriaMovimientosPaginado({
    active: tipo === 'activos' ? 'true' : 'false',
    pagination,
    search,
  });

  useEffect(() => {
    mutateRef.current = mutate;
  }, [mutate, mutateRef]);

  const optionsPagination = useMemo<IListGenericaPagination>(() => {
    return {
      currentPage: pagination.currentPage || 0,
      rowsPerPage: pagination.limite,
      totalItems: total,
      totalPages: Math.ceil(total === 0 ? 1 : total / pagination.limite),
      onPageChange,
      onRowsPerPageChange,
    };
  }, [
    onPageChange,
    onRowsPerPageChange,
    pagination.currentPage,
    pagination.limite,
    total,
  ]);

  const categoriasAsItemList = useMemo(
    () => parseCategoriaMovimientosToItemList(categorias),
    [categorias]
  );

  const onSelect = useCallback(
    (_id: string) => {
      const categoriaS = categorias.find((categoria) => categoria._id === _id);
      setEditCategoria(categoriaS);
    },
    [categorias]
  );

  const onCancel = useCallback(() => {
    setEditCategoria(undefined);
    if (mutateRef.current) mutateRef.current();
  }, [mutateRef]);

  const onSearch = useCallback(
    ({ target }: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setSearch(target.value);
    },
    []
  );

  return (
    <Box display="flex" flex="100%" sx={{ height: '100%' }}>
      <Box
        flex="40%"
        display="flex"
        flexDirection="column"
        alignItems="stretch"
        sx={{
          borderRight: '0.1em solid #EAEAEA',
        }}
      >
        <Box sx={{ mx: 2, mt: 2 }}>
          <Typography variant="h5" component={'div'} sx={{ mb: 2 }}>
            {t('title')}
          </Typography>
          <TextField
            fullWidth
            size="small"
            label={t('search')}
            onChange={debounce(onSearch, 500)}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <SearchOutlined />
                </InputAdornment>
              ),
            }}
          />
        </Box>

        <Box flexGrow={1} sx={{ mx: 0 }}>
          {isLoading ? (
            <FullScreenLoading />
          ) : (
            <ListGeneric
              items={categoriasAsItemList}
              title={t('title')}
              selected={categoriaSelected?._id}
              pagination={optionsPagination}
              emptyText={t('emptyList')}
              onSelect={onSelect}
            />
          )}
        </Box>
      </Box>
      <Box
        flex="60%"
        sx={{ pl: 2, pt: 2 }}
        display="flex"
        flexDirection="column"
        justifyContent="center"
      >
        <Typography variant="h5" component={'div'}>
          {t('details')}
        </Typography>

        <Box height="100%" flex={1} display="flex">
          {!categoriaSelected ? (
            <CategoriaMovimientoDetailPlaceHolder
              message={t('seleccionHelp')}
            />
          ) : (
            <CategoriaMovimientoEditView
              categoria={categoriaSelected}
              onCancel={onCancel}
            />
          )}
        </Box>
      </Box>
    </Box>
  );
};
